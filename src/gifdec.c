...
#define MAX(A, B) ((A) > (B) ? (A) : (B))
  ...
  #define gifdec_calloc   ps_calloc
  #define gifdec_free     free
  #define gifdec_malloc   ps_malloc
  #define gifdec_realloc  ps_realloc
#else
  #define gifdec_calloc   calloc
  #define gifdec_free     free
  #define gifdec_malloc   malloc
  #define gifdec_realloc  realloc
#endif

void* gifdec_malloc(size_t size);
void* gifdec_realloc(void* ptr, size_t new_size);
void gifdec_free(void* ptr);
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    gif = gifdec_malloc(sizeof(gd_GIF) + 5 * width * height);
#elif LV_COLOR_DEPTH == 16
    gif = gifdec_malloc(sizeof(gd_GIF) + 4 * width * height);
#elif LV_COLOR_DEPTH == 8
    gif = gifdec_malloc(sizeof(gd_GIF) + 3 * width * height);
#endif
...
new_table(int key_size)
{
    ...
    Table *table = gifdec_malloc(sizeof(*table) + sizeof(Entry) * init_bulk);
    if (table) {
...
add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
    ...
    if (table->nentries == table->bulk) {
        table->bulk *= 2;
        table = gifdec_realloc(table, sizeof(*table) + sizeof(Entry) * table->bulk);
        if (!table) return -1;
        table->entries = (Entry *) &table[1];
        *tablep = table;
    }
