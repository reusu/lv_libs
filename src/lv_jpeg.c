...
#define SJPEG_FRAME_INFO_ARRAY_OFFSET   22
  ...
  #define sjpg_calloc   ps_calloc
  #define sjpg_free     free
  #define sjpg_malloc   ps_malloc
  #define sjpg_realloc  ps_realloc
#else
  #define sjpg_calloc   calloc
  #define sjpg_free     free
  #define sjpg_malloc   malloc
  #define sjpg_realloc  realloc
#endif

void* sjpg_malloc(size_t size);
void* sjpg_realloc(void* ptr, size_t new_size);
void sjpg_free(void* ptr);
...
static lv_res_t decoder_info( lv_img_decoder_t * decoder, const void * src, lv_img_header_t * header )
{
  ...
  if(src_type == LV_IMG_SRC_VARIABLE) {
      ...
      if(!strncmp((char *)raw_sjpeg_data, "_SJPG__", strlen("_SJPG__") )) {
        ...
        uint8_t *workb_temp = sjpg_malloc( TJPGD_WORKBUFF_SIZE );
...
static size_t input_func ( JDEC* jd, uint8_t* buff, size_t ndata )
{
    ...
    if(io->type == SJPEG_IO_SOURCE_C_ARRAY) {
        ...
        if( buff ) {
            ...
            lv_fs_seek(lv_file_p, ndata, LV_FS_SEEK_CUR);
//            uint32_t pos;
 ...
 static lv_res_t decoder_open( lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc )
{
    ...
    if(dsc->src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if( sjpeg == NULL ) {
           sjpeg =  sjpg_malloc( sizeof( SJPEG ) );
        ...
        if( !strncmp( ( char * ) sjpeg->sjpeg_data, "_SJPG__", strlen("_SJPG__") ) ) {
            ...
            sjpeg->frame_base_array = sjpg_malloc( sizeof(uint8_t *) * sjpeg->sjpeg_total_frames );
            ...
            sjpeg->frame_cache = (void *)sjpg_malloc( sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3/*2*/ );
            ...
            sjpeg->workb =   sjpg_malloc( TJPGD_WORKBUFF_SIZE );
            if( ! sjpeg->workb ) {
                 lv_sjpg_cleanup( sjpeg );
                 sjpeg = NULL;
                 return LV_RES_INV;
            ...
            sjpeg->tjpeg_jd =   sjpg_malloc( sizeof( JDEC ) );
            if( ! sjpeg->tjpeg_jd ) {
                 lv_sjpg_cleanup( sjpeg );
                 sjpeg = NULL;
                 return LV_RES_INV;
            ...
            if(rc == JDR_OK ) {
                ...
                sjpeg->sjpeg_cache_frame_index = -1;
                sjpeg->frame_cache = (void *)sjpg_malloc( sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3 );
                if( ! sjpeg->frame_cache ) {
                    lv_sjpg_cleanup( sjpeg );
                    sjpeg = NULL;
                    return LV_RES_INV;
        ...
        if(!strcmp(&fn[strlen(fn) - 5], ".sjpg")) {
            ...
            if(strcmp((char *)buff, "_SJPG__") == 0 ) {
                ...
                sjpeg->frame_base_array = NULL;//sjpg_malloc( sizeof(uint8_t *) * sjpeg->sjpeg_total_frames );
                sjpeg->frame_base_offset = sjpg_malloc( sizeof(int) * sjpeg->sjpeg_total_frames );
            ...
            if(rc == JDR_OK ) {
                ...
                sjpeg->frame_base_offset =  sjpg_malloc( sizeof(uint8_t *) * sjpeg->sjpeg_total_frames );
                if( ! sjpeg->frame_base_offset ) {
                    lv_fs_close(&lv_file);
                    lv_sjpg_cleanup(sjpeg);
                    return LV_RES_INV;
                ...
                if(dsc->user_data) sjpg_free(dsc->user_data);
                lv_fs_close(&lv_file);
                return  LV_RES_INV;
            }
        }
...
static void lv_sjpg_free( SJPEG* sjpeg )
{
    if(sjpeg->frame_cache) sjpg_free(sjpeg->frame_cache);
